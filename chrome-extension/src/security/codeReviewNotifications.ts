...
/**
 * Desktop notifications are intentionally disabled. Security feedback is shown
 * through the in-page MCP toaster. Returning true keeps existing audit callers
 * compatible without recording a false notification failure.
 */
...
export function registerCodeReviewNotificationListeners(onSent?: NotificationSentHandler): void {
  ...
  chrome.alarms.onAlarm.addListener(alarm => {
    ...
    void (async () => {
      try {
        ...
        const handled = await createSecurityNotification(
  ...
  logger.debug('[CodeReviewNotifications] Alarm listener registered; desktop notifications disabled');
}
