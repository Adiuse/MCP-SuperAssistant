...
function getAutomationState(): AutomationState {
  ...
  if (automation) return { autoExecute: automation.autoExecute === true };
...
function parseApprovedReadCall(text: string): ParsedReadCall | null {
  ...
  const complete = items.some(item => item?.type === 'function_call_end' && String(item?.call_id ?? '') === callId);
...
function parseGitHubDeviceAuthChallenge(result: unknown): GitHubDeviceAuthChallenge | null {
  ...
  return { verificationUrl: GITHUB_DEVICE_LOGIN_URL, userCode: codeMatch?.[1] };
}
...
async function submitInsertedResult(adapter: any, callId: string): Promise<void> {
  ...
  for (let attempt = 1; attempt <= MAX_RESULT_SUBMIT_ATTEMPTS; attempt += 1) {
    // This message is an Extension tool result, never a REAL_USER_PROMPT. The
    // internal-submission guard also records its one-shot DOM fingerprint so a
    // synchronized user-message observer cannot mistake it for a remote prompt.
    const submitted = await runAsCodeReviewInternalSubmission(() => adapter.submitForm());
    if (submitted) {
      await wait(450);
